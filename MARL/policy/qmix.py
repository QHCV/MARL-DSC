...
class QMIX:
    def __init__(self, args):
        ...
        self.model_dir = args.load_model_dir + '/' + args.alg + '/' + args.map #load model
        # 如果存在模型则加载模型
    ...
    def deep_copy(self,policy):
        # new_agent = QMIX(self.args)
        self.eval_rnn.load_state_dict(copy.deepcopy(policy.eval_rnn.state_dict()))
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        with open(self.args.save_path + "/historydata/loss.txt", "a") as f:
            print(loss, file=f)
    ...
    def save_model(self, train_step):
        ...
        torch.save(self.eval_qmix_net.state_dict(), self.args.save_path + "/model" + '/' + num + '_qmix_net_params.pkl')
        torch.save(self.eval_rnn.state_dict(),  self.args.save_path +  '/model' + '/'+ num + '_rnn_net_params.pkl')
